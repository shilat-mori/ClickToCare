...
export async function POST(req: NextRequest) {
    try {
        ...
        await connect();
        //get the user data
        const { username, password, email, aboutMe } = await req.json();
        console.log(`api/newUsers - username: ${username}, password: ${password}, email: ${email}, aboutMe: ${aboutMe}`)
        ...
        const newUser = new NewUser({ username, password: hashedPassword, email, role: UserRole.unauthorized, score: 0, aboutMe, signTime: new Date() });
        await newUser.save();
        //generate a token - unauthorized, since we just signed up
        const token = await generateToken(newUser.userInfo.username, UserRole.unauthorized);
